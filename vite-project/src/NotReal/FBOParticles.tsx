...
import simulationVertexShader from '../assets/fboSimulationVertexShader.glsl';
import simulationFragmentShader from '../assets/fboSimulationFragmentShader.glsl';
...
const useSimulationMaterialGeometry = (size:number) => {
...
interface FBOParticlesProps {
  ...
  simulationMaterialHook: (size: number) => THREE.ShaderMaterial;
}
const FBOParticles: React.FC<FBOParticlesProps> = ({ vertexShader, fragmentShader, particleCount, simulationMaterialHook }) => {

  const size = particleCount || 128;
  const simulationMaterial = simulationMaterialHook(size);
